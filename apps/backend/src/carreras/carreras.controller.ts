...
export class CarrerasController {
  ...
  async remove(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    // SUPERADMIN y ADMIN borran cualquier carrera en cualquier estado. Los operadores (CHARLIE)
    // pueden borrar sus propias carreras en cualquier estado (pendiente, completada, cancelada o perdida).
    if (req.user?.rol !== 'SUPERADMIN' && req.user?.rol !== 'ADMIN') {
      ...
      if (carrera.creadoPorId && carrera.creadoPorId !== req.user?.sub) {
